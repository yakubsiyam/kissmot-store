    ...
    div.innerHTML = `
      <div class="single-product">
        ...
        <div>
          ...
          <p>Rating: <span class="text-danger fw-bold">${product.rating.rate}</span></p><p>Total Rated: ${product.rating.count}</p>
        </div>

        <div>
          <h2>Price: $ ${product.price}</h2>
          <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-danger">Add to cart</button>
          <button id="details-btn" class="btn btn-danger" type="button" data="${product.id}" data-toggle="modal" data-target="#myModal">Details</button>
        </div>
      </div>
      `;
